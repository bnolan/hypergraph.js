...
describe "hypergraph", ->
  ...
  it "should getEdgesForVertex", ->
    ...
    expect(graph.getEdgesForVertex(sad).length).toEqual(0)

describe "edge", ->
  it "should create", ->
    e = new Edge
    expect(e instanceof Edge).toBeTruthy()
  ...
  it "should contain", ->
    ...
    expect(e.contains(v3)).toBeFalsy()
...
describe "sentence example", ->
  ...
  it "should create vertices", ->
    ...
    expect(graph.vertexCount()).toEqual(850)
    
  it "should create edges", ->
    for sentence in CARL_ANNA.split(".")
      
      words = for word in sentence.split(/\b/) when word.match(/^[a-z]+$/) && graph.containsVertex(word)
        graph.getVertex word
        
      graph.addEdge(new Edge words)
      
    expect(graph.edgeCount()).toEqual(146)
    
  it "should find edges for carl", ->
    vertex = graph.getVertex 'carl'
    expect(vertex.getEdges().length).toEqual 6
    
  it "should find edges for anna", ->
    vertex = graph.getVertex 'anna'
    expect(vertex.getEdges().length).toEqual 7

  it "should find edges for the", ->
    vertex = graph.getVertex 'the'
    expect(vertex.getEdges().length).toEqual 77

    
describe "n-gram example", ->
  ...
  it "should create edges", ->
    ...
    expect(graph.edgeCount()).toEqual(2890)

  it "should find edges for carl", ->
    vertex = graph.getVertex 'carl'
    expect(vertex.getEdges().length).toEqual 4

  it "should find edges for anna", ->
    vertex = graph.getVertex 'anna'
    expect(vertex.getEdges().length).toEqual 10

  it "should find edges for the", ->
    vertex = graph.getVertex 'the'
    expect(vertex.getEdges().length).toEqual 139
